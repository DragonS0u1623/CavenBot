...
export default {
    data: new SlashCommandBuilder().setName('slap')
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        axios.get(TenorAPI).then(async response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${interaction.user.displayName} slaps everyone`)
            ...
            if (user != null) embed.setTitle(`${interaction.user.displayName} slaps ${user.displayName}`)
