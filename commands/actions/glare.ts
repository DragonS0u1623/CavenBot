...
const TenorAPI = `https://api.tenor.com/v1/random?q=anime%20glare&key=${process.env.TENORGIF_KEY}&limit=1&media_filter=minimal`
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        axios.get(TenorAPI).then(async response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${interaction.user.displayName} glares at everyone`)
            ...
            if (user != null) embed.setTitle(`${interaction.user.displayName} glares at ${user.displayName}`)
