...
public class LanguageUtils {
	...
	public static String getUserLanguage(User user) {
		Document doc = lang.find(eq("user", user.getId())).projection(include("lang")).first();
		if (doc == null )
			return null;
	...
	public static JsonObject getLanguage(User user, Guild guild) {
		return JsonParser.parseReader(getLanguageFile(getUserOrGuildLanguage(user, guild)))
				.getAsJsonObject();
	}

	public static JsonObject getLanguage(Guild guild) {
		return JsonParser.parseReader(getLanguageFile(getGuildLanguage(guild)))
				.getAsJsonObject();
	}

	public static JsonObject getError(User user, Guild guild) {
		return JsonParser.parseReader(getLanguageFile(getUserOrGuildLanguage(user, guild)))
				.getAsJsonObject().getAsJsonObject("commands").getAsJsonObject("errors");
	}

	public static JsonObject getError(Guild guild) {
		return JsonParser.parseReader(getLanguageFile(getGuildLanguage(guild)))
				.getAsJsonObject().getAsJsonObject("commands").getAsJsonObject("errors");
	}
