...
public class DefineCommand extends Command {
	...
	// 	String word = ctx.getOption("word").getAsString(), api = "https://dictionaryapi.com/api/v3/references/%s/json/%s?key=%s", def = "";
	// 	ctx.deferReply();
	// 	try {
	...
	// 			if (responseCode >= 500) {
	// 				ctx.replyEphemeral("Sorry but an unexpected server error occured with the dictionary API. Try again in a bit.");
	// 				return;
	// 			} else {
	// 				ctx.replyEphemeral("Sorry. An API error occured while trying to find a definition.");
	// 				return;
	// 			}
	// 		}
	...
	// 				return;
	// 			} else {
	// 				ctx.replyEphemeral("Sorry. An API error occured while trying to find the synonyms.");
	// 				return;
	// 			}
	// 		}
	...
	// 		ctx.replyEphemeral("Sorry an error occured while looking up the definition and synonyms");
	// 		return;
	// 	}
	// 	String synonyms = "";
	// 	for (int i = 1; i <= 5; i++)
