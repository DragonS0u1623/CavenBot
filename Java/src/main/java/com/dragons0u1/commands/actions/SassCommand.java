...
public class SassCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20sass", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("sass").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("sass").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public SassCommand() {
		this.name = "sass";
		this.help = "Sends a random gif of someone being sassy";
	}
