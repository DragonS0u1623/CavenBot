...
public class PunchCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20punch", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("punch").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("punch").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public PunchCommand() {
		this.name = "punch";
		this.help = "Sends a random gif of someone being punched";
	}
