...
public class PatCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20pat", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("pat").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("pat").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public PatCommand() {
		this.name = "pat";
		this.help = "Sends a random gif of someone being pat";
	}
