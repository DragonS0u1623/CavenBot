...
public class GlareCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20glare", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("glare").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("glare").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public GlareCommand() {
		this.name = "glare";
		this.help = "Sends a random gif of someone glaring";
	}
