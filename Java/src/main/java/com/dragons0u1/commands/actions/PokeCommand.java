...
public class PokeCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20poke", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("poke").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("poke").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public PokeCommand() {
		this.name = "poke";
		this.help = "Sends a random gif of someone being poked";
	}
