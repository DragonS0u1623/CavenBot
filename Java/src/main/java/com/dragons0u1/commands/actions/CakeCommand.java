...
public class CakeCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "cake", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("cake").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("cake").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public CakeCommand() {
		this.name = "cake";
		this.help = "Sends a random gif of cake";
	}
