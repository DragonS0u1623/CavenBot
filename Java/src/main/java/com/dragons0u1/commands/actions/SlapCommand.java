...
public class SlapCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20slap", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("slap").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("slap").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public SlapCommand() {
		this.name = "slap";
		this.help = "Sends a random gif of someone being slapped";
	}
