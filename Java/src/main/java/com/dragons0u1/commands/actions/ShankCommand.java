...
public class ShankCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "knife", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("shank").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("shank").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public ShankCommand() {
		this.name = "shank";
		this.help = "Sends a random gif of someone being stabbed";
	}

	@Override
	public String[] getAliases() {
		return new String[]{ "stab", "kill" };
	}
