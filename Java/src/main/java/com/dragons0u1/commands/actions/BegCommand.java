...
public class BegCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20beg", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("beg").get("title").getAsString();
	// 		embed.setTitle(String.format(title, ctx.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("beg").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, ctx.getMember().getEffectiveName(),
	...
	public BegCommand() {
		this.name = "beg";
		this.help = "Sends a random gif of someone begging";
	}
