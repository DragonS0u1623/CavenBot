...
public class CuddleCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20cuddle", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("cuddle").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("cuddle").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public CuddleCommand() {
		this.name = "cuddle";
		this.help = "Sends a random gif of someone being cuddled";
	}
