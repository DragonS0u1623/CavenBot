...
public class KissCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20kiss", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("kiss").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("kiss").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public KissCommand() {
		this.name = "kiss";
		this.help = "Sends a random gif of someone being kissed";
	}
