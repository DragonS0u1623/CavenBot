...
public class LickCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "anime%20lick", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("lick").get("title").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("lick").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, e.getMember().getEffectiveName(),
	...
	public LickCommand() {
		this.name = "lick";
		this.help = "Sends a random gif of someone being licked";
	}
