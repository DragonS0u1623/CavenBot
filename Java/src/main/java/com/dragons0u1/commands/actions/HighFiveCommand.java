...
public class HighFiveCommand extends Command {
	...
	// 	String gifURL = "";
	// 	try {
	// 		URL url = new URL(String.format(TENORAPI, "high five", Dotenv.load().get("TENORGIF_KEY")));
	...
	// 		title = action.getAsJsonObject("high_five").get("title").getAsString();
	// 		embed.setTitle(String.format(title, ctx.getMember().getEffectiveName()));
	// 	} else {
	// 		title = action.getAsJsonObject("high_five").get("title_mention").getAsString();
	// 		embed.setTitle(String.format(title, ctx.getMember().getEffectiveName(),
	...
	public HighFiveCommand() {
		this.name = "highfive";
		this.help = "Sends a random gif of a high five";
	}
